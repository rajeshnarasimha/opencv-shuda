...
static const int FREAK_NB_SCALES = CFreak::NB_SCALES;
static const int FREAK_NB_PAIRS = CFreak::NB_PAIRS;
static const int FREAK_NB_ORIENPAIRS = CFreak::NB_ORIENPAIRS;

static const short FREAK_DEF_PAIRS[CFreak::NB_PAIRS] =
{ // default pairs
...
void CFreak::compute( const Mat& image, std::vector<KeyPoint>& keypoints, Mat& descriptors ) {
...
uchar CFreak::meanIntensity( const cv::Mat& image, const cv::Mat& integral,
                            const float kp_x,
                            const float kp_y,
...
vector<int> CFreak::selectPairs(const std::vector<Mat>& images
...
CFreak::CFreak( bool _orientationNormalized, bool _scaleNormalized
...
int CFreak::descriptorSize() const {
    return FREAK_NB_PAIRS / 8; // descriptor length in bytes
}

int CFreak::descriptorType() const {
    return CV_8U;
}

bool CFreak::compare() const {
	bool bTrue = true;
...
void CFreak::downloadKeypoints(const cv::gpu::GpuMat& keypointsGPU, vector<KeyPoint>& keypoints)
{
...
unsigned int CFreak::gpuCompute( const cv::gpu::GpuMat& cvgmImg_, const cv::gpu::GpuMat& cvgmImgInt_, cv::gpu::GpuMat& cvgmKeyPoint_,  cv::gpu::GpuMat* pcvgmDescriptor_ ) {
