...
#include "SemiDenseTrackerFHessFreak.h"
...
btl::image::semidense::CSemiDenseTrackerFHessFreak::CSemiDenseTrackerFHessFreak(unsigned int uPyrHeight_)
	:CSemiDenseTracker(uPyrHeight_)
{
	...
	_pSURF.reset(new cv::gpu::SURF_GPU(5,4,1,false,0.2,true));
}

void btl::image::semidense::CSemiDenseTrackerFHessFreak::initUMax(){
...
void btl::image::semidense::CSemiDenseTrackerFHessFreak::makeRandomPattern(unsigned short usHalfPatchSize_, int nPoints_, cv::Mat* pcvmPattern_)
{
...
void btl::image::semidense::CSemiDenseTrackerFHessFreak::initOrbPattern(){
	// Calc cvmPattern_
...
bool btl::image::semidense::CSemiDenseTrackerFHessFreak::initialize( boost::shared_ptr<cv::gpu::GpuMat> _acvgmShrPtrPyrBW[4] )
{
	_nFrameIdx = 0;
	_sDescriptorByte = 8;
	initUMax();
...
/*
		...
		(*_pSURF)(_cvgmBlurredPrev[n], cv::gpu::GpuMat(), _cvgmKeyPoint[n]);
		unsigned int uFinalSalientPointsAfterNonMax = std::min( (unsigned int) _cvgmKeyPoint[n].cols, _uMaxKeyPointsAfterNonMax[n] );
		//3.sort all salient points according to their strength and pick the first _uTotalParticles;
		btl::device::semidense::thrustSort(_cvgmKeyPoint[n].ptr<float2>(0),_cvgmKeyPoint[n].ptr<float>(2),uFinalSalientPointsAfterNonMax);
...
void btl::image::semidense::CSemiDenseTrackerFHessFreak::track( boost::shared_ptr<cv::gpu::GpuMat> _acvgmShrPtrPyrBW[4] )
{
	for (short n = 3; n>-1; --n ) {
		//processing the frame
