...
int main(int argc, char* argv[])
{
	...
	SURF surf(200,4,2,false,true);
	// detecting keypoints & computing descriptors
	BRISK* pBrisk = new BRISK(30,4); 
	...
	pBrisk->compute( cvmGray1, vKeypoints1, cvmDescriptor1 );

	surf(cvmGray2, cv::Mat(), vKeypoints2);
	double t = (double)getTickCount();

	pBrisk->compute( cvmGray2, vKeypoints2, cvmDescriptor2 );
