...
void normalKeys ( unsigned char key, int x, int y ){
	...
	case 'p':
		...
		_bRenderRawPtsNls =! _bRenderRawPtsNls;
		for(unsigned int i=0; i < _nKFCounter; i++)	{
	...
	case '7':
		...
		break;
	case '8':
		_bRenderVirtualFrame = !_bRenderVirtualFrame;
		glutPostRedisplay();
		break;
	case '0':
		_usViewNO = ++_usViewNO % _nKFCounter; 
...
// store a frame and detect feature points for tracking.
	...
	_pModel->gpuIntegrateFrameIntoVolumeCVCV(*p1stKF,0);
	_pModel->gpuRaycast(*p1stKF,0,&*_pVirtualFrame);
	_vRFIdx.push_back(0);
...
// ( second frame )
    ...
    if ( _bCapture && _nKFCounter < _nReserved ) {
		...
		_pModel->gpuIntegrateFrameIntoVolumeCVCV(*pCurrentKF,0);
		//detect planes
	...
	for( std::vector< btl::kinect::CKeyFrame::tp_shared_ptr* >::iterator cit = _vShrPtrsKF.begin(); cit!= _vShrPtrsKF.end(); cit++,usViewIdxTmp++ ) {
		...
		if(_bRenderRawPtsNls) (**cit)->render3DPtsInWorldCVCV(_pGL.get(), _pGL->_usPyrLevel, _usColorIdx, false );
	}
