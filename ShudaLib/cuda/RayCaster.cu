...
struct RayCaster
{
    ...
    __device__ __forceinline__ bool checkInds (const int3& g) const {
		return (g.x >= 0 && g.y >= 0 && g.z >= 0 && g.x < _cvgmYZxXVolume.rows && g.y < _cvgmYZxXVolume.rows && g.z <_cvgmYZxXVolume.rows);
    }

    __device__ __forceinline__ float readTsdf (int x, int y, int z) const {
		return unpack_tsdf (_cvgmYZxXVolume.ptr (x)[_cvgmYZxXVolume.rows * y + z]);
    }
    ...
    __device__ __forceinline__ float interpolateTrilineary (const float3& point) const  {
		...
		if (g.x <= 0 || g.x >= _cvgmYZxXVolume.rows - 1)		return numeric_limits<float>::quiet_NaN ();
		if (g.y <= 0 || g.y >= _cvgmYZxXVolume.rows - 1)		return numeric_limits<float>::quiet_NaN ();
		if (g.z <= 0 || g.z >= _cvgmYZxXVolume.rows - 1)		return numeric_limits<float>::quiet_NaN ();
